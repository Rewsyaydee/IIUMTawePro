...
export async function getUserRecordByTelegramId(telegramId) {
  const rows = await supabaseRequest(
    `/users?telegram_id=eq.${encodeURIComponent(telegramId)}&select=id,telegram_id,name,role,bureau,status,matric_number,kulliyyah,registration_step,notify_tier,mahallah,committee_prefs&limit=1`
  );
